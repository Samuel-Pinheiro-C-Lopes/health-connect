...
public class AppointmentController {
	...
	@PreAuthorize("hasAuthority(@rolesConfiguration.admin) or hasAuthority(@rolesConfiguration.manager) or hasAuthority(@rolesConfiguration.doctor)")
	public ResponseEntity<List<AppointmentResponseDTO>> findByDoctorId(@PathVariable final Long id) {
		return ResponseEntity.ok(this.appointmentService.findByDoctorId(id));
	}

	@GetMapping("/patient/{id}")
	@PreAuthorize("hasAuthority(@rolesConfiguration.admin) or hasAuthority(@rolesConfiguration.manager) or hasAuthority(@rolesConfiguration.doctor)")
	public ResponseEntity<List<AppointmentResponseDTO>> findByPatientId(@PathVariable final Long id) {
		return ResponseEntity.ok(this.appointmentService.findByPatientId(id));
	}
