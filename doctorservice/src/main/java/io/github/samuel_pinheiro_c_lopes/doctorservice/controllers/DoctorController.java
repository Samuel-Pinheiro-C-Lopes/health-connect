...
public class DoctorController {
	...
	@PreAuthorize("hasAuthority(@rolesConfiguration.admin) or hasAuthority(@rolesConfiguration.manager) or hasAuthority(@rolesConfiguration.doctor)")
	public ResponseEntity<DoctorDetailsResponseDTO> findDetailsById(@PathVariable final Long id) {
		return ResponseEntity.ok(this.doctorService.findDetailsById(id));
	}
	@GetMapping("/status/{status}")
	@PreAuthorize("hasAuthority(@rolesConfiguration.admin) or hasAuthority(@rolesConfiguration.manager)")
	public ResponseEntity<List<DoctorResponseDTO>> findByStatus(@PathVariable final StatusSolicitacao status) {
		return ResponseEntity.ok(this.doctorService.findByStatus(status));
	}
	@PutMapping("/{id}/status/{status}")
	@PreAuthorize("hasAuthority(@rolesConfiguration.admin) or hasAuthority(@rolesConfiguration.manager)")
	public ResponseEntity<DoctorResponseDTO> updateStatus(
			@PathVariable final Long id,
			@PathVariable final StatusSolicitacao status
	) {
		return ResponseEntity.ok(this.doctorService.updateStatus(id, status));
	}
    @PostMapping
    @PreAuthorize("hasAuthority(@rolesConfiguration.admin) or hasAuthority(@rolesConfiguration.manager)")
